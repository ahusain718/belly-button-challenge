...
function buildCharts(sample) {
  d3.json("https://static.bc-edx.com/data/dl-1-2/m14/lms/starter/samples.json").then((data) => {
    ...
    sortedData = sortedData.sort((a, b) => b.sample_value - a.sample_value).slice(0, 10).reverse();
    
    // Extract the top 10 OTUs
    let topOtuIds = sortedData.map(d => `OTU ${d.otu_id}`);
    let topOtuLabels = sortedData.map(d => d.otu_label);
    let topSampleValues = sortedData.map(d => d.sample_value);
